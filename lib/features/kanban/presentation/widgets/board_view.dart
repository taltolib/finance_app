...
class _BoardViewState extends State<BoardView> {
      ...
      floatingActionButton: _deleteMode
        ...
        label: Text(
          _selectedCardIds.isEmpty
              ? 'Выберите карточки'
              : 'Удалить (${_selectedCardIds.length})',
      ...
      body: Container(
        ...
        child: Column(
          children: [
                ...
                child: Row(
                  children: [
                    const Icon(Icons.info_outline,
                        color: KanbanUiColors.red, size: 16),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        'Нажмите на карточку для выбора, затем нажмите "Удалить"',
                        style: kanbanText(size: 12, color: KanbanUiColors.red),
                      ),
                    ),
                    GestureDetector(
            ...
            Expanded(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  final boardHeight =
                  (constraints.maxHeight - (_boardPadding * 2))
                      .clamp(260.0, double.infinity);
                  final contentWidth = (totalItems * _columnWidth) +
                      ((totalItems - 1) * _columnSpacing);
                                    ...
                                    return Padding(
                                      ...
                                      child: SizedBox(
                                        ...
                                        child: KanbanColumnWidget(
                                          ...
                                          onAddCard: () =>
                                              _addCardToColumn(column),
                                          onEditColumn:
                                              (columnId, currentTitle) {
                                            _showRenameDialog(
                                                context, columnId, currentTitle);
                                          },
            ...
            // ─── Dots indicator (скролл между колонками) ─────────────────
            _DotsIndicator(
              total: totalItems,
              currentIndex: _currentColumnIndex.clamp(0, totalItems - 1),
  ...
  void _showRenameDialog(
      BuildContext context, String columnId, String currentTitle) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Переименовать колонку',
            style: kanbanText(size: 17, weight: FontWeight.w700)),
        content: TextField(
            ...
            children: [
              Expanded(
                child: ElevatedButton(
                  ...
                  child: Text('Добавить',
                      style:
                      kanbanText(size: 13, weight: FontWeight.w700)),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
